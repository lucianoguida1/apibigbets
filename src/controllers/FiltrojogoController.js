...
class FiltrojogoController extends Controller {
    ...
    async deleteFiltroJogo(req, res) {
        try {
            const { id } = req.params;
            ...
            if (!id) {
                return res.status(400).json({
                    status: "error",
                    message: "O ID do filtro de jogo é obrigatório",
                    errorCode: 400
                });
            }

            const filtrojogo = await filtrojogos.deleteFiltroJogo(id);

            if (!filtrojogo) {
                return res.status(404).json({
                    status: "error",
                    message: "Filtro de jogo não encontrado",
                    errorCode: 404
                });
            }
            ...
            return res.status(200).json({
                status: "success",
                message: "Filtro de jogo deletado com sucesso",
                statusCode: 200
            });
        } catch (error) {
            return res.status(500).json({
